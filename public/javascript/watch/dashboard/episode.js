...
},{}],"iB90z":[function(require,module,exports) {
var _model = require("../../models/model");
var _episodeView = require("../../views/dashboard/episodeView");
async function controlEpisode(id) {
    const url = `/media/episode/hard/${id}`;
    const data = _episodeView.getData();
    const response = await (0, _model.patchFull)(url, data);
    _episodeView.renderEpisode(response, id);
}
//
async function controlLink(id) {
    const data = _episodeView.getLinkData();
    const response = id ? await (0, _model.patchFull)(`/media/link/${id}`, data) : await (0, _model.postFull)("/media/link", data);
    _episodeView.renderLink(response);
}
async function controlDeleteLink(id) {
    await (0, _model.deleteRequest)(`/media/link/${id}`);
    _episodeView.renderDeleteLink(id);
}
async function init() {
    _episodeView.initialize();
    _episodeView.handleEpisode(controlEpisode);
    _episodeView.handleLink(controlLink);
    _episodeView.handleDeleteLink(controlDeleteLink);
}
init();

},{"../../models/model":"4SBHD","../../views/dashboard/episodeView":"cuyrO"}],"4SBHD":[function(require,module,exports) {
...
function initialize() {
    (0, _independent.softUpdate)("soft-update", "episode");
    ...
    window.addEventListener("keydown", (ev)=>{
        const { code } = ev;
        if (code !== "KeyO") return;
        (0, _utils.openPopup)("link-popup", undefined, undefined, afterOpenPopup);
    });
}
/*
...
function onEditLink() {
    container && container.addEventListener("click", (ev)=>{
        ...
        (0, _utils.openPopup)("link-popup", editLinkPopup, [
            currentLink
        ]);
    });
}
