...
})({"dD1dy":[function(require,module,exports) {
...
module.bundle.HMR_BUNDLE_ID = "fcd7434ccd89faa9";
"use strict";
...
},{}],"fdvUo":[function(require,module,exports) {
var _model = require("../../models/model");
var _seasonView = require("../../views/dashboard/seasonView");
//
async function controlUpdateSeason(id) {
    const data = _seasonView.getData();
    const response = await (0, _model.patchFull)(`/media/season/${id}`, data);
    _seasonView.renderUpdateSeason(response);
}
//
async function controlEpisode(action = "soft") {
    const url = action === "hard" ? `/media/episode/hard` : "/media/episode/soft";
    const data = _seasonView.getEpisodeData(action);
    const response = await (0, _model.postFull)(url, data);
    _seasonView.renderEpisode(response);
}
//
async function controlDeleteEpisode(id) {
    await (0, _model.deleteRequest)(`/media/episode/${id}`);
    _seasonView.renderDeleteEpisode(id);
}
async function init() {
    _seasonView.initialize();
    _seasonView.handleUpdateSeason(controlUpdateSeason);
    _seasonView.handleEpisode(controlEpisode);
    _seasonView.handleDeleteEpisode(controlDeleteEpisode);
}
init();

},{"../../models/model":"4SBHD","../../views/dashboard/seasonView":"h2MdX"}],"4SBHD":[function(require,module,exports) {
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"h2MdX":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
/*
...
*/ // ============================== RENDERES
parcelHelpers.export(exports, "renderUpdateSeason", ()=>renderUpdateSeason);
//
parcelHelpers.export(exports, "renderEpisode", ()=>renderEpisode);
...
*/ // ============================== HANDLERS
parcelHelpers.export(exports, "handleUpdateSeason", ()=>handleUpdateSeason);
parcelHelpers.export(exports, "handleEpisode", ()=>handleEpisode);
...
let currentEpisode;
function renderUpdateSeason(response) {
    (0, _utils.alertResponse)(response.message);
    window.setTimeout(()=>window.location.reload(), 3000);
}
function renderEpisode(response) {
    ...
    if (next) next.insertAdjacentHTML("beforebegin", (0, _utils.dlinkCard)(episode, "Episode ", undefined, redirect));
    else if (prev) prev.insertAdjacentHTML("afterend", (0, _utils.dlinkCard)(episode, "Episode ", undefined, redirect));
    else container.insertAdjacentHTML("afterbegin", (0, _utils.dlinkCard)(episode, "Episode ", undefined, redirect));
    document.querySelector("body").dataset.next = +episode.episode + 1;
    document.getElementById("episode").value = +episode.episode + 1;
}
function renderDeleteEpisode(id) {
...
function getEpisodeData(action) {
    const { id: season, series } = (0, _utils.parseQuery)(window.location.search);
...
function handleUpdateSeason(controlUpdateSeason) {
    const { id: season } = (0, _utils.parseQuery)(window.location.search);
...
function handleEpisode(controlEpisode) {
    ...
    async function formHandler(ev) {
        ...
        try {
            ...
            (0, _utils.stopRotateBtn)(btnid);
            (0, _utils.closePopup)(popupid, clearEpisodePopup, [
                action
            ]);
        } catch (error) {
            (0, _utils.displayError)(error, btnid);
...
function handleDeleteEpisode(controlDeleteEpisode) {
    container && container.addEventListener("click", async (ev)=>{
...
function mediaCard(media) {
    const link = media.type === "collection" ? `/media?collection=${media.id}` : `/d/${media.type.slice(0, 1)}/${media.id}`;
    const markup = `
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3","./functions":"hZORM"}]},["dD1dy","fdvUo"], "fdvUo", "parcelRequiree8ef")
