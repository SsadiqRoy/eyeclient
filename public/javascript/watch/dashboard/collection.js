...
},{}],"9eyAz":[function(require,module,exports) {
var _model = require("../../models/model");
var _collectionView = require("../../views/dashboard/collectionView");
//
async function controlUpdateCollection(id) {
    const data = _collectionView.getData();
    const response = await (0, _model.patchFull)(`/media/hard/${id}`, data);
    _collectionView.renderUpdateCollection(response);
}
async function controlCollection(data) {
    const response = await (0, _model.patchFull)(`/media/collection`, data);
    _collectionView.renderCollection(response);
}
async function init() {
    _collectionView.initialize();
    _collectionView.handleUpdateCollection(controlUpdateCollection);
    _collectionView.handleCollection(controlCollection);
}
init();
